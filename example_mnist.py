...
mu, std =gp.predict(x_test[0:n_test])
"""
yp=np.zeros(n_test)
std=np.zeros(n_test)
gp1=GaussianProcessRegressor(kernel,optimizer=None)
gp1.fit(x_train[:500],y_train[:500])
mu,stdt=gp1.predict(x_test,return_std=True)
"""
print("GP Regression:")
print( np.sqrt( np.mean( (np.rint(mu)-y_test[:n_test])**2 ) ) ) 
